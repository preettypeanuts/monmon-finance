...
interface CategoryFormDialogProps {
  ...
  /** Mobile: stack inside settings categories nested drawer. */
...
export function CategoryFormDialog({
  ...
  const form = (
    ...
    >
      {useNestedDrawer ? (
        <>
          <div className={FORM_DIALOG_BODY_SCROLL}>{formFields}</div>
          <div className={FORM_DIALOG_FOOTER}>{formActions}</div>
        </>
      ) : (
        <>
          ...
          <ResponsiveDialogFooter>{formActions}</ResponsiveDialogFooter>
        </>
      )}
    </form>
  );
